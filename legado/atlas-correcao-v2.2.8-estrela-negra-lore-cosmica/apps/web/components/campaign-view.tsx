...
export function CampaignView({ campaignId }: { campaignId: string }) {
  ...
  const tabs: LoreTab[] = ['BIBLIA', 'RESUMO', 'NARRACAO', 'LOCAIS', 'RECURSOS', 'RUMORES', 'FAUNA', 'AMEACAS', 'HISTORIA', 'HORROR'];
            ...
            <>
              ...
              {selectedHex.discoveryStatus === 'DESCONHECIDO' && isPlayerView ? (
                ...
                <>
                  ...
                  {activeTab === 'BIBLIA' && <CampaignBibleTab campaign={campaign} isMaster={Boolean(isMaster)} onCopy={copyNarration} />}
...
function CampaignBibleTab({ campaign, isMaster, onCopy }: { campaign: Campaign; isMaster: boolean; onCopy: (text?: string) => Promise<void> }) {
  ...
  return <section className="sidebar-section campaign-bible">
    ...
    <LoreCard title="A Estrela Negra" text={bible.blackStar.publicDescription} secret={isMaster ? bible.blackStar.secretNature : undefined} />
    <LoreCard title={`${bible.catastrophe.name} · ${bible.catastrophe.wizardName}`} text={`${bible.catastrophe.wizardTitle} tentou realizar ${bible.catastrophe.experimentName} em ${bible.catastrophe.siteName}. ${bible.catastrophe.intention}`} secret={isMaster ? bible.catastrophe.failure : undefined} />
    <LoreCard title={bible.portal.name} text={bible.portal.publicLore} secret={isMaster ? bible.portal.secretMechanism : undefined} />
    ...
    <LoreCard title={bible.sunEater.name} text={`${bible.sunEater.epithets.join(' · ')}. ${bible.sunEater.publicSigns.join(' ')}`} secret={isMaster ? `${bible.sunEater.truth ?? ''} ${bible.sunEater.impossibleBattle ?? ''}`.trim() : undefined} />
    <div className="bible-section"><h3>O que o mundo sabe</h3><ul>{bible.playerKnowledge.map((item) => <li key={item}>{item}</li>)}</ul></div>
    <div className="bible-section"><h3>Regiões narrativas</h3>{bible.regions.map((region) => <LoreCard key={region.id} title={`${region.name} · ${region.epithet}`} text={`${region.publicDescription} Recurso raro: ${region.rareResource}. Mistério: ${region.centralMystery}`} secret={isMaster ? `${region.secret ?? ''} Facção: ${region.dominantFaction}. Criatura emblemática: ${region.signatureCreature}.` : undefined} />)}</div>
    <div className="bible-section"><h3>Facções</h3>{bible.factions.map((faction) => <LoreCard key={faction.name} title={faction.name} text={`${faction.publicGoal} Método: ${faction.method}.`} secret={isMaster ? `${faction.secretGoal ?? ''} ${faction.relationToPortal ?? ''}`.trim() : undefined} />)}</div>
    <div className="bible-section"><h3>Relíquias</h3>{bible.relics.map((relic) => <LoreCard key={relic.name} title={relic.name} text={`${relic.appearance} ${relic.publicLegend}`} secret={isMaster ? `${relic.truePurpose ?? ''} Custo: ${relic.cost ?? 'desconhecido'}.` : undefined} />)}</div>
    <div className="bible-section"><h3>Atos da campanha</h3>{bible.campaignActs.map((act) => <LoreCard key={act.act} title={`Ato ${act.act} · ${act.title}`} text={act.objective} secret={isMaster ? `${act.revelations?.join(' ') ?? ''} Pressão: ${act.failurePressure ?? ''}` : undefined} />)}</div>
    {isMaster && <>
