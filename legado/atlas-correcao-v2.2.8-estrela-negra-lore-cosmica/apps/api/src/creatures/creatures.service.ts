...
import { CosmicArchetype, CosmicMutation, CosmicOrigin, CosmicTemperament, generateOriginalCreature } from './generation/original-creature-generator';
...
export class CreaturesService implements OnModuleInit {
  ...
  async selectForEncounter(options: {
    ...
    cosmicTemperament?: CosmicTemperament;
    monsterLevel?: number;
    ...
    return Array.from({ length: amount }, (_, index) => generateOriginalCreature(
      ...
      base,
      {
        archetype: options.cosmicArchetype,
        origin: options.cosmicOrigin,
        mutation: options.cosmicMutation,
        temperament: options.cosmicTemperament,
        monsterLevel: options.monsterLevel
      }
    ));
  }

  async saveGeneratedCosmicCreatures(campaignId: string, blocks: CreatureStatBlock[]) {
    const cosmicBlocks = blocks.filter((block) => block.theme === CreatureTheme.COSMIC);
    if (cosmicBlocks.length === 0) return [];
    const entities = cosmicBlocks.map((block) => {
      const statBlock: CreatureStatBlock = {
        ...block,
        source: CreatureSource.CUSTOM,
        theme: CreatureTheme.COSMIC,
        license: 'Criatura de horror cósmico gerada proceduralmente pelo Atlas das Cinzas.'
      };
      return this.creaturesRepository.create({
        campaignId,
        source: CreatureSource.CUSTOM,
    ...
    return this.creaturesRepository.save(entities, { chunk: 50 });
  }
