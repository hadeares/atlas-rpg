...
export function generateEncounter(
  ...
  let loreConnection = `O encontro utiliza elementos de ${lore.title}, na região ${lore.region?.name ?? 'desconhecida'}, e pode revelar sinais da Estrela Negra.`;
  ...
  switch (category) {
    ...
    case EncounterCategory.HORROR: {
      if (primaryCreature) {
        title = `${primaryCreature.name} em ${lore.title}`;
        publicNarration = `${primaryCreature.narration} ${pick(lore.horror.omens, next)} ${lore.campaignConnections?.blackStarSign ?? 'A luz do sol parece perder força ao redor da criatura.'} O terreno reage à presença como se reconhecesse algo que não deveria existir.`;
        truth = `${primaryCreature.originStory ?? lore.horror.truth} ${primaryCreature.portalConnection ?? lore.campaignConnections?.portalEcho ?? ''} A criatura está ligada a ${lore.horror.name} e sua aparição indica que a rede do portal perdeu estabilidade.`.trim();
        objective = primaryCreature.behavior;
        behavior = `${primaryCreature.behavior} ${primaryCreature.tactics}`;
        participants = [{
          name: primaryCreature.name,
          type: 'HORROR_COSMICO',
          count: String(Math.max(1, includedCreatures.length)),
          role: primaryCreature.cosmicConcept ?? 'Aberração cósmica personalizada'
        }];
        peacefulSolutions = [
          `Explorar a fraqueza: ${primaryCreature.weakness}`,
          `Usar a contenção local: ${lore.horror.containment}`,
          'Romper o padrão ambiental que permite a manifestação.',
          'Recuar mantendo contato visual e evitando separar o grupo.'
        ];
        rewards = [...primaryCreature.rewards, 'Uma pista direta sobre a origem da manifestação local.'];
        statBlockSuggestion = `Use a ficha completa de ${primaryCreature.name}, ND ${primaryCreature.challengeLabel}, CA ${primaryCreature.armorClass} e ${primaryCreature.hitPoints} PV.`;
        loreConnection = `${primaryCreature.cosmicConcept ?? 'A criatura'} manifesta o mesmo padrão de ${lore.horror.name} em ${lore.region?.name ?? lore.title}. Está ligada a ${lore.campaignConnections?.linkedFaction ?? 'uma força desconhecida'}, à relíquia ${lore.campaignConnections?.linkedRelic ?? 'não identificada'} e à pista: ${lore.campaignConnections?.objectiveClue ?? 'encontrar uma forma de fechar o portal'}. ${lore.masterGuide?.activeConflict ?? ''}`.trim();
      } else {
        title = `${lore.horror.name}: ${formatEnum(lore.horror.stage)}`;
        publicNarration = `Por alguns segundos, o ambiente deixa de obedecer ao que deveria ser normal. ${pick(lore.horror.omens, next)} ${lore.horror.playerEffect} ${lore.campaignConnections?.solarDecaySign ?? ''} Um detalhe impossível permanece visível apenas tempo suficiente para que todos tenham certeza de que aconteceu.`;
        ...
        loreConnection = `${lore.masterGuide?.activeConflict ?? 'A manifestação está ligada ao segredo central do hexágono.'} ${lore.campaignConnections?.portalEcho ?? ''} A pista útil para a campanha é: ${lore.campaignConnections?.objectiveClue ?? 'a manifestação não deve ser combatida como a entidade maior; é preciso romper sua ligação com o portal'}.`.trim();
      }
      break;
    }
