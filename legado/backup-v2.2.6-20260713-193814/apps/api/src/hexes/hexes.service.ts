...
export class HexesService implements OnModuleInit {
  ...
  private async enrichOutdatedLore() {
    ...
    const seeds = new Map(campaigns.map((campaign) => [campaign.id, campaign.seed]));
    let migrated = 0;

    while (true) {
      const outdated = await this.hexesRepository
        .createQueryBuilder('hex')
        .where("COALESCE((hex.state->'lore'->>'schemaVersion')::integer, 0) < 3")
        .take(50)
        .getMany();
      ...
      for (const hex of outdated) {
        const seed = seeds.get(hex.campaignId);
        if (!seed) continue;
        this.applyGeneratedLore(hex, seed);
      }
    ...
    if (migrated > 0) this.logger.log(`${migrated} hexágonos antigos foram enriquecidos para a lore v3.`);
  }

  private applyGeneratedLore(hex: Hex, seed: string) {
    const previous = (hex.state?.lore ?? {}) as Partial<HexLore>;
    const generated = generateHexLore(seed, hex.q, hex.r, hex.terrain, hex.biome, hex.dangerLevel, hex.cosmicInfluence);
    const lore: HexLore = {
      ...generated,
  ...
  async findAll(userId: string, campaignId: string) {
    ...
    const rows = await this.hexesRepository
      .createQueryBuilder('hex')
      .select('hex.id', 'id')
  ...
  async findOne(userId: string, campaignId: string, q: number, r: number) {
    ...
    if (Number(lore?.schemaVersion ?? 0) < 3) {
      hex = await this.hexesRepository.save(this.applyGeneratedLore(hex, access.campaign.seed));
    }
  ...
  async updateLore(userId: string, campaignId: string, q: number, r: number, dto: UpdateHexLoreDto) {
    ...
    return this.hexesRepository.save(hex);
  }
  ...
  async regenerateLore(userId: string, campaignId: string, q: number, r: number) {
    ...
    const lore = generateHexLore(`${access.campaign.seed}:v${loreVersion}`, q, r, hex.terrain, hex.biome, hex.dangerLevel, hex.cosmicInfluence);
