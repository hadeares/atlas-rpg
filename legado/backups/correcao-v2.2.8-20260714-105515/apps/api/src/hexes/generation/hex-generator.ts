...
export function generateHex(seed: string, q: number, r: number): GeneratedHexData {
  if (q === 0 && r === 0) {
    return {
      q,
      r,
      ...
      state: {
        ...
        startingHex: true,
        visitCount: 0,
        lore: generateHexLore(seed, q, r, TerrainType.PLANICIE, BiomeType.CAMPOS_CINZENTOS, 2, 10)
      }
    };
  }
  ...
  return {
    ...
    state: {
      ...
      visitCount: 0,
      lore: generateHexLore(seed, q, r, terrain, getBiome(terrain), dangerLevel, cosmicInfluence)
    }
  };
}

export function generateHexGrid(seed: string, radius: number) {
  ...
  for (let q = -radius; q <= radius; q += 1) {
    ...
    for (let r = minimumR; r <= maximumR; r += 1) {
      hexes.push(generateHex(seed, q, r));
    }
  }
