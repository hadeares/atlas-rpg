...
export type CosmicArchetype = 'RANDOM' | 'STALKER' | 'BRUTE' | 'CONTROLLER' | 'ARTILLERY' | 'SWARM' | 'BOSS';
export type CosmicOrigin = 'RANDOM' | 'FERIDA' | 'VAZIO' | 'SONHO' | 'CRISTAL' | 'TEMPO' | 'PARASITA' | 'ABISMO';
...
function resolveOrigin(origin: CosmicOrigin | undefined, next: () => number): Exclude<CosmicOrigin, 'RANDOM'> {
  ...
  return pick(['FERIDA', 'VAZIO', 'SONHO', 'CRISTAL', 'TEMPO', 'PARASITA', 'ABISMO'] as const, next);
}
...
function resolveArchetype(archetype: CosmicArchetype | undefined, cr: number, next: () => number): Exclude<CosmicArchetype, 'RANDOM'> {
  ...
  const options: Array<Exclude<CosmicArchetype, 'RANDOM'>> = ['STALKER', 'BRUTE', 'CONTROLLER', 'ARTILLERY', 'SWARM'];
...
function cosmicTraits(origin: Exclude<CosmicOrigin, 'RANDOM'>, archetype: Exclude<CosmicArchetype, 'RANDOM'>, cr: number, dc: number, next: () => number): CreatureTrait[] {
  ...
  const originTraits: Record<Exclude<CosmicOrigin, 'RANDOM'>, CreatureTrait[]> = {
  ...
  const archetypeTraits: Record<Exclude<CosmicArchetype, 'RANDOM'>, CreatureTrait[]> = {
...
function cosmicActions(
  ...
  const originActions: Record<Exclude<CosmicOrigin, 'RANDOM'>, CreatureAction> = {
  ...
  actions.push(originActions[origin]);

  const archetypeActions: Record<Exclude<CosmicArchetype, 'RANDOM'>, CreatureAction> = {
  ...
  if (cr >= 2) actions.push(archetypeActions[archetype]);
  return actions;
}

function cosmicBonusActions(archetype: Exclude<CosmicArchetype, 'RANDOM'>, cr: number): CreatureAction[] {
  if (cr < 2) return [];
  const options: Record<Exclude<CosmicArchetype, 'RANDOM'>, CreatureAction> = {
...
export function generateOriginalCreature(
  ...
  const cosmicDescription = `${name} apresenta ${silhouette}. ${capitalize(surface)}; ${anatomy}. ${capitalize(face)}. Quando se desloca, ${movement}. Sua presença altera o ambiente: ${aura}. É ${descriptor}.`;
  const cosmicNarration = `Antes que a criatura seja vista, vocês ouvem ${voice} e percebem um cheiro de ${odor}. Então ${silhouette} atravessa o limite da visão. ${capitalize(surface)} e ${anatomy}. ${capitalize(face)}. Ela não caminha como um animal: ${movement}. Ao redor dela, ${aura}.`;
  ...
  return {
    ...
    signs: resolvedTheme === CreatureTheme.COSMIC
      ? pickMany(['sombras apontando para direções diferentes', 'objetos vibrando sem vento', 'lembranças breves que não pertencem aos personagens', 'animais nascendo com marcas simétricas', 'ecos respondendo antes do som', 'trilhas que terminam em superfícies verticais', 'água formando círculos sem impacto', 'sonhos idênticos entre pessoas distantes'], 4, next)
      : resolvedTheme === CreatureTheme.INFECTED
    ...
    tactics: resolvedTheme === CreatureTheme.COSMIC
      ? `Usa ${originConcepts[origin].power}, escolhe alvos que falharam em testes mentais e explora ${archetype === 'ARTILLERY' ? 'linhas longas de visão' : archetype === 'CONTROLLER' ? 'passagens estreitas e posições elevadas' : archetype === 'STALKER' ? 'escuridão e separação' : 'terreno difícil e medo'}.`
    ...
    cosmicConcept: resolvedTheme === CreatureTheme.COSMIC ? `${originConcepts[origin].label} · ${archetypeLabel(archetype)}` : undefined,
    combatPhases: phases
  };
}
