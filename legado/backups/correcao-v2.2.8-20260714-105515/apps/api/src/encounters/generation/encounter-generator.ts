  ...
  switch (category) {
    ...
    case EncounterCategory.HORROR: {
      if (primaryCreature) {
        ...
        publicNarration = `${primaryCreature.narration} ${pick(lore.horror.omens, next)} O terreno ao redor reage à presença como se reconhecesse algo que não deveria existir.`;
        truth = `${primaryCreature.originStory ?? lore.horror.truth} A criatura está ligada a ${lore.horror.name} e sua aparição indica que a contenção local perdeu estabilidade.`;
        ...
        loreConnection = `${primaryCreature.cosmicConcept ?? 'A criatura'} manifesta o mesmo padrão de ${lore.horror.name}. ${lore.masterGuide?.activeConflict ?? ''}`.trim();
      } else {
