...
export class CampaignsService implements OnModuleInit {
  ...
  async findAll(userId: string) {
    ...
    return memberships.map((membership) => {
      ...
      return {
        ...membership.campaign,
        seed: playerView ? '' : membership.campaign.seed,
        radius: playerView ? 0 : membership.campaign.radius,
        hexCount: playerView
  ...
  async findOne(userId: string, campaignId: string) {
    ...
    return {
      ...access.campaign,
      seed: playerView ? '' : access.campaign.seed,
      radius: playerView ? 0 : access.campaign.radius,
      hexCount,
      memberCount,
