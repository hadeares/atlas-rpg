...
export class EncountersService {
  ...
  async generate(userId: string, campaignId: string, dto: GenerateEncounterDto) {
    ...
    const requestedCategory = dto.category ?? EncounterCategory.ALEATORIO;
    ...
    const creatureMode = dto.category === EncounterCategory.HORROR && !dto.creatureMode
    ...
    const creatureBlocks = shouldGenerateCreatures
      ? await this.creaturesService.selectForEncounter({
          ...
          mode: creatureMode,
          targetCr: dto.targetCr ?? this.recommendedCr(dto.averageLevel ?? 3, dto.partySize ?? 4, dto.intensity),
          amount: dto.creatureCount ?? 1,
          seed: generationSeed,
          hex
        })
      : [];
    const generated = generateEncounter(generationSeed, hex, lore, dto, creatureBlocks);
    ...
    const encounter = await this.encountersRepository.save(this.encountersRepository.create({
      ...
      generationSeed,
      generatorOptions: {
        requestedCategory: dto.category ?? EncounterCategory.ALEATORIO,
