...
export const HexMap = memo(function HexMap({
  ...
  function getBaseScale() {
    ...
    return Math.max(0.01, Math.min(
      Math.max(1, width - canvasPadding * 2) / geometry.width,
      Math.max(1, height - canvasPadding * 2) / geometry.height
    ));
  }
  ...
  drawRef.current = () => {
    ...
    for (const position of geometry.positions) {
      ...
      const unknown = playerView && position.hex.discoveryStatus === 'DESCONHECIDO';
      const color = unknown ? terrainPalettes.DESCONHECIDO[0] : position.color;
    ...
    if (statsRef.current) {
      statsRef.current.textContent = `${visible.length.toLocaleString('pt-BR')} de ${geometry.positions.length.toLocaleString('pt-BR')} desenhados`;
    }
  };
  ...
  function centerHex(q: number, r: number) {
    const position = geometry.coordinateMap.get(`${q},${r}`);
    if (!position) return;
    const zoom = Math.max(2.2, viewRef.current.zoom);
