...
export class HexesService implements OnModuleInit {
  ...
  private async enrichOutdatedLore() {
    ...
    while (true) {
      const outdated = await this.hexesRepository
        ...
        .where("COALESCE((hex.state->'lore'->>'schemaVersion')::integer, 0) < 4")
        .take(50)
        .getMany();
    ...
    if (migrated > 0) this.logger.log(`${migrated} hexágonos antigos foram enriquecidos para a lore v4.`);
  }
  ...
  async findOne(userId: string, campaignId: string, q: number, r: number) {
    ...
    if (Number(lore?.schemaVersion ?? 0) < 4) {
